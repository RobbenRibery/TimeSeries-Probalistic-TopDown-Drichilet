            ...
            for it in tr:
                ...
                for b in range(n_batches):
                    ...
                    for batch_index in range(input_batch.shape[0]):
                        ...
                        decoder_batch_ouputs[batch_index] = decoder_batch_ouputs[batch_index]
                        ...
                        attention_batch_outputs[batch_index] = attention_batch_outputs[batch_index]
                        ...
                        model_batch_outputs[batch_index] = model_output
                        model_batch_outputs[batch_index] = model_batch_outputs[batch_index]
                        ...
                        #print(model_output, target)
                        loss = get_loss(model_output, target, 0.001)
                        #print(loss)
                    ...
                    ##### write an if loop for inspectiing the running loss: 
                    ##
                    ##
                    ##### 
                ...
                #writer.add_scalar('Loss/train - epoch', batch_losses, n_batches)

                tr.set_postfix(loss="{0:.3f}".format(sum(batch_losses)))

            writer.close()

        return losses

def train_model(model, input_tensor, target_tensor, n_epochs, target_len, batch_size,learning_rate=0.067,):
        ...
        with trange(n_epochs) as tr:
            ...
            for it in tr:
                ...
                for b in range(n_batches):
                    ...
                    decoder_batch_ouputs = zeros(input_batch.shape[0],target_len, no_child,self.lstm_output_dim,)
                    attention_batch_outputs = zeros(input_batch.shape[0],target_len,no_child,self.mha_output_dim,)
                    model_batch_outputs = zeros(input_batch.shape[0],target_len,no_child,self.model_ouput_dim,)
